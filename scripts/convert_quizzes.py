...
def generate_quiz_html(quiz_number: int, questions: list) -> str:
    ...
    for q in questions:
        ...
        # Tentar encontrar explicação no texto da pergunta se não estiver nas opções
        explanation = ""
        
        for opt in q['options']:
            correct_attr = 'true' if opt['correct'] else 'false'
            
            # Se o texto da opção contém "Explicação:", extraímos
            opt_text = opt['text']
            if "*Explicação:*" in opt_text:
                parts = opt_text.split("*Explicação:*")
                opt_display_text = parts[0].strip()
                explanation = parts[1].strip().strip("*")
            else:
                opt_display_text = opt_text

            feedback = f"✅ Correto! {explanation}" if opt['correct'] else f"❌ Incorreto. {explanation}"
            if not explanation:
                feedback = f"✅ Correto!" if opt['correct'] else f"❌ Incorreto. Tente novamente."
            
            html_parts.append(
                f'  <div class="quiz-option" data-correct="{correct_attr}" '
                f'data-feedback="{feedback}">{opt_display_text}</div>\n'
            )
